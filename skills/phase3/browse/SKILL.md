...
description: |
  ...
  site", "take a screenshot", or "dogfood this". (robobuilder-local)
triggers:
  - browse a page
  - headless browser
...
```
Scale must be 1-3 (browser tool cap). Changing `--scale` recreates the browser context; refs from `snapshot` are invalidated (rerun `snapshot`), but `load-html` content is replayed automatically. Not supported in headed mode.
...
`$B` is the browse binary resolved during setup, or the host-provided browser automation tool when no local binary is installed.
