...
description: |
  ...
  "land it", "ship it to production". (robobuilder-local)
allowed-tools:
  - Bash
  - Read
  - Write
...
```bash
B=""
[ -x "bin/browse" ] && B="bin/browse"
[ -z "$B" ] && command -v browse >/dev/null 2>&1 && B="$(command -v browse)"
if [ -n "$B" ]; then
  echo "READY: $B"
else
  echo "NEEDS_BROWSER_TOOL"
fi
```

If `NEEDS_BROWSER_TOOL` appears, use the host-provided browser automation tool if available. Do not install third-party browser binaries from this skill.
...
```bash
bin/robobuilder-review-read 2>/dev/null
```
...
```bash
cat skills/phase4/diff-review/checklist.md 2>/dev/null || echo "Checklist not found"
```
...
Then run `robobuilder-diff-scope` to classify the changes:

```bash
eval $(bin/robobuilder-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
...
```bash
$B snapshot -i -a -o ".robobuilder/deploy-reports/post-deploy.png"
```
...
```bash
mkdir -p .robobuilder/deploy-reports
```
...
Save report to `.robobuilder/deploy-reports/{date}-pr{number}-deploy.md`.
