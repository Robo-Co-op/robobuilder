...
description: |
  ...
  is ready, asks about deploying, wants to push code up, or asks to create a PR. (robobuilder-local)
allowed-tools:
  - Bash
  - Read
  - Write
...
For Design Review: run `source <(bin/robobuilder-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 9, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
[ -f .robobuilder/no-test-bootstrap ] && echo "BOOTSTRAP_DECLINED"
```
...
If user picks H → write `.robobuilder/no-test-bootstrap` and continue without tests.
...
If user picks C → write `.robobuilder/no-test-bootstrap`. Tell user: "If you change your mind later, delete `.robobuilder/no-test-bootstrap` and re-run." Continue without tests.
...
- Create an issue assigned to that person (use the platform detected in Step 0):
  - **If GitHub:**
    ...
    gh issue create \
      ...
      --body "Found failing on branch <current-branch>. Failure is pre-existing.\n\n**Error:**\n```\n<first 10 lines>\n```\n\n**Last modified by:** <author>\n**Noticed by:** RoboBuilder /ship on <date>" \
...
- Review report sections (`## ROBOBUILDER REVIEW REPORT`)
...
> RoboBuilder can search learnings from your other projects on this machine to find
...
This makes the compounding visible. The user should see that RoboBuilder is getting
...
Check if the diff touches frontend files using `robobuilder-diff-scope`:

```bash
source <(bin/robobuilder-diff-scope <base> 2>/dev/null)
```
...
```bash
bin/robobuilder-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
```bash
source <(bin/robobuilder-diff-scope <base> 2>/dev/null) || true
...
```bash
bin/robobuilder-specialist-stats 2>/dev/null || true
```
...
1. **Testing** — read `skills/phase4/diff-review/specialists/testing.md`
2. **Maintainability** — read `skills/phase4/diff-review/specialists/maintainability.md`
...
3. **Security** — if SCOPE_AUTH=true, OR if SCOPE_BACKEND=true AND DIFF_LINES > 100. Read `skills/phase4/diff-review/specialists/security.md`
4. **Performance** — if SCOPE_BACKEND=true OR SCOPE_FRONTEND=true. Read `skills/phase4/diff-review/specialists/performance.md`
5. **Data Migration** — if SCOPE_MIGRATIONS=true. Read `skills/phase4/diff-review/specialists/data-migration.md`
6. **API Contract** — if SCOPE_API=true. Read `skills/phase4/diff-review/specialists/api-contract.md`
7. **Design** — if SCOPE_FRONTEND=true. Use the existing design review checklist at `skills/phase4/diff-review/design-checklist.md`

### Adaptive gating

After scope-based selection, apply adaptive gating based on specialist hit rates:

For each conditional specialist that passed scope gating, check the `robobuilder-specialist-stats` output above:
...
```bash
bin/robobuilder-learnings-search --type pitfall --query "{specialist domain}" --limit 5 2>/dev/null || true
```
...
1. The red-team checklist from `skills/phase4/diff-review/specialists/red-team.md`
...
```bash
bin/robobuilder-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
...
OLD_CFG=$(bin/robobuilder-config get codex_reviews 2>/dev/null || true)
...
```bash
bin/robobuilder-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
bin/robobuilder-learnings-log '{"skill":"ship","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
3. **Queue-aware version pick (workspace-aware ship, v1.6.4.0+).** Call `bin/robobuilder-next-version` to see what's already claimed by open PRs + active sibling Conductor worktrees, then render the queue state to the user:

   ```bash
   QUEUE_JSON=$(bun run bin/robobuilder-next-version \
     --base <base> \
     --bump "$BUMP_LEVEL" \
...
- Message: "RoboBuilder recommends maintaining a TODOS.md organized by skill/component, then priority (P0 at top through P4, then Completed at bottom). See TODOS-format.md for the full format. Would you like to create one?"
...
```bash
# Export [robobuilder-context] blocks from all WIP commits on this branch.
# This file becomes input to the CHANGELOG entry and may inform PR body context.
mkdir -p "$(git rev-parse --show-toplevel)/.robobuilder"
git log <base>..HEAD --grep="^WIP:" --format="%H%n%B%n---END---" > \
  "$(git rev-parse --show-toplevel)/.robobuilder/wip-context-before-squash.md" 2>/dev/null || true
```
...
> You are executing the /document-release workflow after a code push. Read the full skill file `skills/phase5/handoff/SKILL.md` and execute its complete workflow end-to-end, including CHANGELOG clobber protection, doc exclusions, risky-change gates, and named staging. Do NOT attempt to edit the PR body — no PR exists yet. Branch: `<branch>`, base: `<base>`.
>
...
**Always update the PR title to start with `v$NEW_VERSION`.** PR titles use the workspace-aware format `v<NEW_VERSION> <type>: <summary>` — version ALWAYS first, no exceptions, no "custom title kept intentionally" escape hatch. The shared helper `bin/robobuilder-pr-title-rewrite.sh` is the single source of truth for the rule.

1. Read the current title: `CURRENT=$(gh pr view --json title -q .title)` (or `glab mr view -F json | jq -r .title`).
2. Compute the corrected title: `NEW_TITLE=$(bin/robobuilder-pr-title-rewrite.sh "$NEW_VERSION" "$CURRENT")`. The helper handles three cases: title already correct (no-op), title has a different `v<X.Y.Z.W>` prefix (replace it), or title has no version prefix (prepend one).
...
# (See Step 19 idempotency block + bin/robobuilder-pr-title-rewrite.sh for the rule.)
